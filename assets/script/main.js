...
function decimal2base(dec, y, b) {
  ...
  if (id >= 0 && nd > 0) {
    txt =
      "Multiply the number with the destination base raised to the power of decimals of the result (up to 6 digits resolution) :";
    txt +=
      "<div class='mt-2'> floor(" +
      dec +
      "&times;" +
      b +
      "<sup>" +
  ...
  for (let n = dec, k = 0; n >= 1; k++) {
    row = "<tr>";
    row +=
      "<td class='border-gray-400 border-t px-3 py-2'>" + n + "/" + b + "</td>";
    row +=
      "<td class='border-gray-400 border-t px-1 py-2 text-center'>" +
      Math.floor(n / b) +
      "</td>";
    row +=
      "<td class='border-gray-600 border py-2 px-1 border-dashed bg-slate-200 text-center'>" +
      (n % b).toFixed() +
      "</td>";
    row +=
      "<td class='border-gray-400 border-t py-2 px-1 text-center'>" +
      k +
      "</td>";
    row += "</tr>";
    n = Math.floor(n / b);
...
function updateURLParams() {
  ...
  params.set("x", encodeURIComponent(xelem.value));
...
function onconvert() {
  ...
  try {
    ...
    xelem.classList.add("bg-current"); // Use CSS variable for background color
  } catch (err) {
    xelem.classList.add("bg-red-50"); // Use CSS variable for error background color
    yelem.value = "";
    return;
  }
  var dec = y.toString(10);
  y = y.toString(b2).toUpperCase().trim();
  yelem.value = y;
